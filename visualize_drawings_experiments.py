...
DEFAULT_EXPERIMENTS_DIRECTORY = "experiments_iterative/outputs"
DOMAINS = "domains"
...
CODEX_SAMPLE_LANGUAGE_VISUALIZATION = "codex_sample_language_visualization.png"
...
parser.add_argument(
    "--iterations", nargs="+", default=[0],
)
parser.add_argument(
    "--visualize_language_results",
    action="store_true",
    help="Visualize programs generated by Codex and language.",
)
parser.add_argument(
    "--visualize_codex_results",
    action="store_true",
...
def get_prompted_programs(args, codex_results, max_to_display=10):
    # Get programs in the original prompt.
    if "programs" not in list(codex_results["params"]["body_task_types"]):
        return [], []

    should_get_language = args.visualize_language_results and "language" in list(
        codex_results["params"]["body_task_types"]
    )

    prompt_language, prompt_programs = [], []
    sample_programs_to_final_task_language = defaultdict(list)
    for query in codex_results["results_by_query"]:
        for body_task_data in query["prompt"]["body_task_data"]:
            ...
            if p is not None:
                ...
                if should_get_language:
                    prompt_language.append(body_task_data["task_language"])

        if (
            query["prompt"]["final_task_data"]["task_program"] is not None
            and not should_get_language
        ):
            prompt_programs.append(query["prompt"]["final_task_data"]["task_program"])
        else:
            final_task_language = query["prompt"]["final_task_data"]["task_language"]
            prompt_completion_programs = set(
                [q["text"] for q in query["completion"]["choices"]]
            )
            for completion_program in prompt_completion_programs:
                sample_programs_to_final_task_language[completion_program].append(
                    final_task_language
                )
    ...
    if should_get_language:
        prompt_programs_language = random.sample(
            list(zip(prompt_programs, prompt_language)),
            min(max_to_display, len(prompt_programs)),
        )

        prompt_programs, prompt_language = zip(*prompt_programs_language)
    else:
        prompt_programs = random.sample(
            prompt_programs, min(max_to_display, len(prompt_programs)),
        )
    return (
        [Program.parse(string_program) for string_program in prompt_programs],
        prompt_language,
        sample_programs_to_final_task_language,
    )
...
def visualize_codex_results(args, experiment_type, replication_directory):
    ...
    for batch_directory in batch_directories:
        ...
        for iteration in args.iterations:
            ...
            (
                ...
                prompted_language,
                sample_programs_to_final_task_language,
            ) = get_prompted_programs(args, codex_results)
            prompted_language = (
                [] if not args.visualize_language_results else prompted_language
            )
            if len(prompted_programs) > 0:
                prompted_progams_montage = drawings_primitives.display_programs_as_grid(
                    ...
                    ncols=8,
                    # array_captions=prompted_language,
                )
                codex_results_figure.append(prompted_progams_montage)
                if args.visualize_language_results:
                    print(
                        f"Codex prompt program examples: {batch_directory}, stitch_iteration={iteration}"
                    )
                    for idx, caption in enumerate(prompted_language):
                        print(f"{idx}: {caption}")

            # Montage the programs sampled from Codex.
            sampled_programs = get_sampled_programs(codex_results)
            if len(sampled_programs) > 0 and sampled_programs is not None:
                ...
                if sampled_programs_montage is not None:
                    codex_results_figure.append(sampled_programs_montage)

                if args.visualize_language_results:
                    print(
                        f"Codex SAMPLED examples: {batch_directory}, stitch_iteration={iteration}"
                    )
                    for idx, sampled_program in enumerate(sampled_programs):
                        if (
                            str(sampled_program)
                            in sample_programs_to_final_task_language
                        ):
                            print(
                                f"{idx}: {sample_programs_to_final_task_language[sampled_program]}"
                            )
            # Add a line spacer.
    ...
    visualization_file = (
        CODEX_SAMPLE_LANGUAGE_VISUALIZATION
        if args.visualize_language_results
        else CODEX_SAMPLE_VISUALIZATION
    )
    codex_visualization_output = os.path.join(replication_directory, visualization_file)
    ...
    for domain in args.domains:
        for experiment_type in args.experiment_types:
            ...
            for replication_directory in replication_directories:
                ...
                if args.visualize_codex_results or args.visualize_language_results:
                    visualize_codex_results(
